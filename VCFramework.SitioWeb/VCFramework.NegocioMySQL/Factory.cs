...
namespace VCFramework.NegocioMySQL
{
    public class Factory
    {
        ...
        public int Delete<T>(T objeto)
        {
            ...
            using (OdbcCommand cmd = new OdbcCommand(string.Format("DELETE FROM {0} WHERE ID = {1}", obj.NombreTabla, valorId), con))
            {
                con.Open();
                cmd.ExecuteNonQuery();
                filasAfectadas++;
        ...
        public int Update<T>(T objeto)
        {
            ...
            query.AppendFormat(" WHERE ID = {0}", valorId.ToString());
            ...
            try
            {
                ...
                if (listaTablas != null && listaTablas.Count > 0)
                {
                    ...
                    foreach (string s in listaTablas)
                    {
                        ...
                        string[] tablitas = s.Split('_');
                        string prefijo = tablitas[0].ToUpper();//ac
                        string sufijo = tablitas[1].ToUpper();//acciones
                        if (tablitas.Length > 2)
                            sufijo = sufijo + tablitas[2].ToUpper();
                        //if (nombreEntidad.ToUpper().Contains(prefijo))//AutentificacionUsuario
                        //    cantidad++;
                        //if (nombreEntidad.ToUpper().Contains(sufijo))
                        if (nombreEntidad.ToUpper() == sufijo)
                            cantidad++;
